...
const databaseFilename = '/localfirstv1.sqlite3'

let db

const initializeSQLite = async () => {
	if (db) {
		console.log('using open db')
		return db
	}
	try {
		let database
		const sqlite3 = await sqlite3InitModule({ print: log, printErr: error });
		console.log('Initialized SQLite3', sqlite3.version.libVersion);
		if ('opfs' in sqlite3) {
			database = new sqlite3.oo1.OpfsDb(databaseFilename)
			console.log(`OPFS is available, created persisted database at ${database.filename}`)
		} else {
			database = new sqlite3.oo1.DB(databaseFilename, 'ct');
			console.log(`OPFS is not available, created transient database ${database.filename}`)
		}
		db = database
		return database
	} catch (error) {
...
Comlink.expose({getDb, db, initializeSQLite})

// Comlink.expose({db, getDb, exec, query, pull, deleteAll})
