...
import {toFilename, fetchRemoteTracks, remoteTrackToTrack} from './utils.js'
import {setupDatabase, getTracks, upsertTrack} from './database.js'
import {downloadTrack} from './utils-node.js'
...
if (!values.folder)
	console.log('--folder <path> chooses the parent folder for your radio here (a new folder will be created)')
...
if (incomingTracks.length) {
	...
	if (values.pull) {
		...
		console.log('Successfully pulled', incomingTracks.length, 'tracks from remote Radio4000')
	}
} else {
